    ...
    class Solution {
        public String solution(String my_string, int m, int c) {
             ...
             for (int i = 0; i < my_string.length() / m; i++) {
                 answer += my_string.substring(m * i , m + (m * i)).substring(c - 1, c);
             }
             return answer;
        }

        public String solution2(String my_string, int m, int c) {
            return IntStream.range(0,(my_string.length() / m)).mapToObj(i -> my_string.substring(m * i, m + (m * i)).substring(c - 1, c)).collect(Collectors.joining());
        }
    }
}
