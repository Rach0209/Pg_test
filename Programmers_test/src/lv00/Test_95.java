...
public class Test_95 {
    class Solution {
        public String[] solution(String my_string) {
            // return Arrays.asList(Arrays.stream(my_string.trim().split(" ")).filter(i -> i.length() > 0).toArray()).toArray(new String[0]);
            // return Arrays.stream(my_string.trim().split(" ")).filter(i -> !i.equals("")).toArray(String[]::new);
