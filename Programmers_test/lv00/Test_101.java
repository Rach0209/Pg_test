...
public class Test_101 {
    public static void main(String[] args) {
        ...
        System.out.println(Integer.compare(arr1.length, arr2.length) != 0 ?
                (Arrays.stream(arr1).sum() > Arrays.stream(arr2).sum() ? 1 : -1) : 0);
    }
    class Solution {
        public int solution(int[] arr1, int[] arr2) {
            if (arr1.length > arr2.length) {
                ...
                return Arrays.stream(arr1).sum() >= Arrays.stream(arr2).sum() ? (Arrays.stream(arr1).sum() == Arrays.stream(arr2).sum() ? 0 : 1) : -1;
            } else {
                return -1;
            }
            // 괴랄한 답을 만들었따.
//            return arr1.length > arr2.length ? 1 : (arr1.length < arr2.length ? -1 : (Arrays.stream(arr1).sum() > Arrays.stream(arr2).sum() ? 1 : (Arrays.stream(arr1).sum() == Arrays.stream(arr2).sum() ? 0 : -1)));
        }
    }
}
